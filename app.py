...
def get_price_history(price,ticker,min_rows=120):
    """
    Freshness-aware + same-date-revision-aware price loader.

    Important:
    Yahoo can publish an unfinished/stale daily candle first and revise the
    SAME trading date later after market close. Therefore comparing dates
    ...
    # Always re-check when local last EOD is before today's calendar date.
    # On the morning after a trading day this catches same-date Yahoo revisions.
    needs_freshness_check=(
        local.empty or
        local_last is None or
        pd.Timestamp(local_last).date() < now.date() or
        len(local)<min_rows
    )
    ...
    if not remote.empty:
        remote=remote.copy().sort_values("Date")
        remote["Date"]=pd.to_datetime(remote["Date"])
        remote_last=remote["Date"].max()

        if local.empty:
            return remote,f"Histori terbaru diambil saat dibuka · EOD {pd.Timestamp(remote_last).date()}"

        # Case 1: remote has a genuinely newer trading date.
        if pd.Timestamp(remote_last) > pd.Timestamp(local_last):
            return remote,f"Histori diperbarui saat dibuka · EOD {pd.Timestamp(remote_last).date()}"

        # Case 2: SAME DATE but Yahoo revised the EOD candle after close.
        if pd.Timestamp(remote_last) == pd.Timestamp(local_last):
            lrow=local.loc[local["Date"]==local_last].iloc[-1]
            rrow=remote.loc[remote["Date"]==remote_last].iloc[-1]
            ...
            lclose,rclose=_num(lrow,"Close"),_num(rrow,"Close")
            lhigh,rhigh=_num(lrow,"High"),_num(rrow,"High")
            llow,rlow=_num(lrow,"Low"),_num(rrow,"Low")
            lvol,rvol=_num(lrow,"Volume"),_num(rrow,"Volume")

            price_changed=(
                (pd.notna(lclose) and pd.notna(rclose) and abs(rclose-lclose) > 0.001) or
                (pd.notna(lhigh) and pd.notna(rhigh) and abs(rhigh-lhigh) > 0.001) or
                (pd.notna(llow) and pd.notna(rlow) and abs(rlow-llow) > 0.001)
            )
            volume_changed=(
                pd.notna(lvol) and pd.notna(rvol) and abs(rvol-lvol) > max(1000,abs(lvol)*0.001)
            )

            if price_changed or volume_changed:
                return remote,(
                    f"EOD direvisi dari sumber saat dibuka · "
                    f"{pd.Timestamp(remote_last).date()} · "
                    f"Close lokal {rupiah(lclose)} → terbaru {rupiah(rclose)}"
                )

    if len(local)>=min_rows:
        last_txt="" if local_last is None else f" · EOD {pd.Timestamp(local_last).date()}"
        return local,"Histori lokal lengkap"+last_txt

    if not local.empty:
        last_txt="" if local_last is None else f" · EOD {pd.Timestamp(local_last).date()}"
        return local,"Histori lokal masih pendek"+last_txt
...
def technical_frame(df):
    ...
    # Long-term moving-average cross signals.
    x["GoldenCrossEvent"]=(x["SMA50"]>x["SMA200"])&(x["SMA50"].shift(1)<=x["SMA200"].shift(1))
    x["DeathCrossEvent"]=(x["SMA50"]<x["SMA200"])&(x["SMA50"].shift(1)>=x["SMA200"].shift(1))
    x["GoldenCrossActive"]=x["SMA50"]>x["SMA200"]
    x["BullMAAlignment"]=(x["Close"]>x["SMA20"])&(x["SMA20"]>x["SMA50"])&(x["SMA50"]>x["SMA200"])
    x["BearMAAlignment"]=(x["Close"]<x["SMA20"])&(x["SMA20"]<x["SMA50"])&(x["SMA50"]<x["SMA200"])
    return x
...
def technical_glossary():
    return {
        ...
        "Golden Cross": "Golden Cross terjadi ketika SMA50 menembus naik SMA200. Ini sinyal perbaikan tren jangka menengah-panjang, bukan jaminan harga naik.",
        "Death Cross": "Death Cross terjadi ketika SMA50 menembus turun SMA200. Ini peringatan bahwa tren jangka menengah-panjang sedang melemah."
    }
...
def _quarter_yoy(series):
    # latest quarter versus same quarter last year
    if series is None or len(series)<5:
        return np.nan
    cur=float(series.iloc[0]); prev=float(series.iloc[4])
    if prev==0:
        return np.nan
    return cur/prev-1

def _ttm_growth(series):
    # latest 4 quarters versus previous 4 quarters
...
def _extract_info_currency(obj):
    ...
    if not price_currency:
        try:
            ...
            price_currency=getattr(fi,"currency",None)
        except Exception:
            pass

    if not price_currency:
        try:
            md=obj.get_history_metadata() or {}
            price_currency=md.get("currency")
        except Exception:
            pass

    return (
        (financial_currency or price_currency or "IDR").upper(),
        (price_currency or financial_currency or "IDR").upper(),
        company,
        sector,
        industry,
        info
...
def _fx_rate(from_currency,to_currency,date=None):
    ...
    try:
        ...
        if date is not None:
            ...
            h=obj.history(start=start,end=end,auto_adjust=False)
            if not h.empty:
                # nearest observation to requested date
                idx=pd.to_datetime(h.index).tz_localize(None) if getattr(h.index,"tz",None) is not None else pd.to_datetime(h.index)
                distances=np.abs((idx-pd.Timestamp(d).tz_localize(None)).days)
...
def _ttm_dividend_yield(obj,price):
    ...
    try:
        ...
        if not isinstance(a.index,pd.DatetimeIndex):
            return np.nan
        now=pd.Timestamp.now(tz=a.index.tz) if a.index.tz is not None else pd.Timestamp.now()
...
def _sanity_flags(f):
    flags=[]
    checks=[
        ("PER",f.get("trailingPE"),500),
        ("PBV",f.get("priceToBook"),50),
        ("EV/EBITDA",f.get("enterpriseToEbitda"),200),
        ("EV/Sales",f.get("enterpriseToRevenue"),100),
        ("DER",f.get("debtToEquity"),10),
    ]
    for label,v,limit in checks:
        if pd.notna(v) and abs(v)>limit:
            flags.append(f"{label} sangat ekstrem ({v:.1f}x). Periksa currency/unit/periode.")
    if f.get("financialCurrency")!=f.get("priceCurrency") and not f.get("fxApplied",False):
        flags.append("Mata uang laporan berbeda dengan harga saham tetapi FX belum berhasil diterapkan.")
    if pd.notna(f.get("marketCap")) and pd.notna(f.get("equity")):
        implied_pb=_safe_div(f.get("marketCap"),f.get("equity"))
        if pd.notna(implied_pb) and implied_pb>50:
            flags.append("Market cap vs equity menghasilkan PBV ekstrem; cek normalisasi mata uang.")
    return flags

def _reconcile_secondary(info,calc):
    """
    Compare dashboard-calculated ratios with Yahoo info ratios when present.
    This is a secondary sanity comparison, not an official benchmark.
    """
    out={}
    pairs={
        "PER":("trailingPE","trailingPE"),
        "PBV":("priceToBook","priceToBook"),
        "EV/EBITDA":("enterpriseToEbitda","enterpriseToEbitda"),
        "EV/Sales":("enterpriseToRevenue","enterpriseToRevenue"),
        "ROE":("returnOnEquity","returnOnEquity"),
        "Margin":("profitMargins","profitMargins"),
    }
    for label,(info_key,calc_key) in pairs.items():
        try:
            sec=float(info.get(info_key)) if info.get(info_key) is not None else np.nan
        except Exception:
            sec=np.nan
        cv=calc.get(calc_key,np.nan)
        if pd.notna(sec) and pd.notna(cv):
            denom=max(abs(sec),1e-9)
            diff=abs(cv-sec)/denom
            status="Dekat" if diff<=0.20 else "Berbeda sedang" if diff<=0.50 else "Berbeda besar"
            out[label]={"calculated":cv,"secondary":sec,"diff":diff,"status":status}
    return out


def _ratio_plausibility_score(pe,pb,ev_ebitda,ev_sales):
    score=0.0
    for v,lim in [(pe,500),(pb,50),(ev_ebitda,200),(ev_sales,100)]:
        if pd.isna(v):
            continue
        av=abs(v)
        if av>lim:
            score += math.log10(max(av/lim,1))*5
    return score

def _secondary_ratio_error(info,pe,pb,ev_ebitda,ev_sales):
    pairs=[
        ("trailingPE",pe),
        ("priceToBook",pb),
    ...
    errs=[]
    for key,calc in pairs:
        try:
            sec=float(info.get(key)) if info.get(key) is not None else np.nan
        except Exception:
            sec=np.nan
        if pd.notna(sec) and sec!=0 and pd.notna(calc):
            errs.append(abs(calc-sec)/max(abs(sec),1e-9))
    return float(np.mean(errs)) if errs else np.nan

def _candidate_valuation(
    market_cap,price_currency,statement_currency,flow_date,balance_date,
    revenue_raw,net_income_raw,ebitda_raw,equity_raw,debt_raw,cash_raw
):
    fx_flow=_fx_rate(statement_currency,price_currency,flow_date)
    fx_balance=_fx_rate(statement_currency,price_currency,balance_date)
    if statement_currency==price_currency:
        fx_flow=fx_balance=1.0

    revenue=_convert_value(revenue_raw,fx_flow)
    net_income=_convert_value(net_income_raw,fx_flow)
    ebitda=_convert_value(ebitda_raw,fx_flow)
    equity=_convert_value(equity_raw,fx_balance)
    debt=_convert_value(debt_raw,fx_balance)
    cash=_convert_value(cash_raw,fx_balance)
    ...
    return {
        ...
        "debt":debt,
        "cash":cash,
        "pe":pe,"pb":pb,"evEbitda":ev_ebitda,"evSales":ev_sales
    }

def _infer_statement_currency(
    reported_currency,price_currency,info,market_cap,
    flow_date,balance_date,
    revenue_raw,net_income_raw,ebitda_raw,equity_raw,debt_raw,cash_raw
):
    candidates=[]
    for cur in [reported_currency,price_currency,"USD"]:
        if cur and cur not in candidates:
            candidates.append(cur)

    evaluated=[]
    for cur in candidates:
        c=_candidate_valuation(
            market_cap,price_currency,cur,flow_date,balance_date,
            revenue_raw,net_income_raw,ebitda_raw,equity_raw,debt_raw,cash_raw
        )
        if pd.isna(c["fxFlow"]) or pd.isna(c["fxBalance"]):
            continue
        plaus=_ratio_plausibility_score(c["pe"],c["pb"],c["evEbitda"],c["evSales"])
        secerr=_secondary_ratio_error(info,c["pe"],c["pb"],c["evEbitda"],c["evSales"])
        combined=(secerr*10 if pd.notna(secerr) else 0)+plaus
        c["plausibility"]=plaus
        c["secondaryError"]=secerr
        c["combinedScore"]=combined
        evaluated.append(c)

    if not evaluated:
        return reported_currency,"metadata",None

    evaluated.sort(key=lambda x:x["combinedScore"])
    best=evaluated[0]
    reported=next((x for x in evaluated if x["currency"]==reported_currency),None)

    method="metadata"
    if reported is None:
        method="inferred"
    elif best["currency"]!=reported_currency:
        reported_score=reported["combinedScore"]
        best_score=best["combinedScore"]
        if reported_score>0 and best_score <= reported_score*0.35:
            method="inferred from ratio reconciliation"
        else:
            best=reported

    return best["currency"],method,best

@st.cache_data(ttl=21600,show_spinner=False)
def fetch_fundamental_snapshot(ticker):
    """
    Currency-aware and period-aware statement engine.
    Statement source remains Yahoo Finance (secondary).
    Official IDX/company filings remain the final verification source.
    """
    try:
    ...
    reported_financial_currency,price_currency,company,sector,industry,info=_extract_info_currency(obj)
    financial_currency=reported_financial_currency
    ...
    a_capex=_statement_row(ac,[
        "Capital Expenditure","Capital Expenditures",
        "Purchase Of PPE","Purchases Of Property Plant And Equipment",
        "Net PPE Purchases"
    ])

    # Raw statement values in financial currency
    revenue_raw=_sum_latest(q_rev,4) if q_rev is not None and len(q_rev)>=4 else _latest_value(a_rev)
    net_income_raw=_sum_latest(q_ni,4) if q_ni is not None and len(q_ni)>=4 else _latest_value(a_ni)
    op_income_raw=_sum_latest(q_op,4) if q_op is not None and len(q_op)>=4 else _latest_value(a_op)
    gross_profit_raw=_sum_latest(q_gp,4) if q_gp is not None and len(q_gp)>=4 else _latest_value(a_gp)
    ebitda_raw=_sum_latest(q_ebitda,4) if q_ebitda is not None and len(q_ebitda)>=4 else _latest_value(a_ebitda)
    cfo_raw=_sum_latest(q_cfo,4) if q_cfo is not None and len(q_cfo)>=4 else _latest_value(a_cfo)
    capex_raw=_sum_latest(q_capex,4) if q_capex is not None and len(q_capex)>=4 else _latest_value(a_capex)
    fcf_raw=np.nan if pd.isna(cfo_raw) or pd.isna(capex_raw) else cfo_raw+capex_raw

    equity_raw=_latest_value(q_equity) if q_equity is not None else _latest_value(a_equity)
    assets_raw=_latest_value(q_assets) if q_assets is not None else _latest_value(a_assets)
    debt_raw=_latest_value(q_debt) if q_debt is not None else _latest_value(a_debt)
    cash_raw=_latest_value(q_cash) if q_cash is not None else _latest_value(a_cash)
    current_assets_raw=_latest_value(q_current_assets) if q_current_assets is not None else _latest_value(a_current_assets)
    current_liab_raw=_latest_value(q_current_liab) if q_current_liab is not None else _latest_value(a_current_liab)

    # Period-normalized growths
    revenue_q_yoy=_quarter_yoy(q_rev)
    earnings_q_yoy=_quarter_yoy(q_ni)
    revenue_ttm_growth=_ttm_growth(q_rev)
    earnings_ttm_growth=_ttm_growth(q_ni)
    revenue_fy_growth=_annual_growth(a_rev)
    earnings_fy_growth=_annual_growth(a_ni)

    # Currency inference + normalization
    balance_date=_series_statement_date(qb) or _series_statement_date(ab)
    flow_date=_series_statement_date(qi) or _series_statement_date(ai)

    price,market_cap,shares=_get_market_price_and_cap(obj,ticker)

    inferred_currency,currency_method,currency_candidate=_infer_statement_currency(
        reported_financial_currency,price_currency,info,market_cap,
        flow_date,balance_date,
        revenue_raw,net_income_raw,ebitda_raw,equity_raw,debt_raw,cash_raw
    )
    financial_currency=inferred_currency or reported_financial_currency

    fx_balance=_fx_rate(financial_currency,price_currency,balance_date)
    fx_flow=_fx_rate(financial_currency,price_currency,flow_date)
    if financial_currency==price_currency:
        fx_balance=fx_flow=1.0

    revenue=_convert_value(revenue_raw,fx_flow)
    net_income=_convert_value(net_income_raw,fx_flow)
    op_income=_convert_value(op_income_raw,fx_flow)
    gross_profit=_convert_value(gross_profit_raw,fx_flow)
    ebitda=_convert_value(ebitda_raw,fx_flow)
    cfo=_convert_value(cfo_raw,fx_flow)
    capex=_convert_value(capex_raw,fx_flow)
    fcf=_convert_value(fcf_raw,fx_flow)

    equity=_convert_value(equity_raw,fx_balance)
    assets=_convert_value(assets_raw,fx_balance)
    debt=_convert_value(debt_raw,fx_balance)
    cash=_convert_value(cash_raw,fx_balance)
    current_assets=_convert_value(current_assets_raw,fx_balance)
    current_liab=_convert_value(current_liab_raw,fx_balance)

    # Previous bases for ROE/ROA; convert with same balance FX approximation
    eq_prev_raw=float(q_equity.iloc[4]) if q_equity is not None and len(q_equity)>=5 else (float(a_equity.iloc[1]) if a_equity is not None and len(a_equity)>=2 else np.nan)
    asset_prev_raw=float(q_assets.iloc[4]) if q_assets is not None and len(q_assets)>=5 else (float(a_assets.iloc[1]) if a_assets is not None and len(a_assets)>=2 else np.nan)
    eq_prev=_convert_value(eq_prev_raw,fx_balance)
    asset_prev=_convert_value(asset_prev_raw,fx_balance)
    ...
    fx_applied=(financial_currency==price_currency) or (pd.notna(fx_flow) and pd.notna(fx_balance))

    result={
        "company":company,"sector":sector,"industry":industry,
        "statementBasis":statement_basis,
        "reportedFinancialCurrency":reported_financial_currency,
        "financialCurrency":financial_currency,
        "currencyDetectionMethod":currency_method,
        "priceCurrency":price_currency,
        "fxFlow":fx_flow,"fxBalance":fx_balance,"fxApplied":fx_applied,
        "marketCap":market_cap,"price":price,
        "revenue":revenue,"netIncome":net_income,"operatingIncome":op_income,"ebitda":ebitda,
        "revenueGrowth":revenue_ttm_growth if pd.notna(revenue_ttm_growth) else revenue_fy_growth,
        "earningsGrowth":earnings_ttm_growth if pd.notna(earnings_ttm_growth) else earnings_fy_growth,
        "revenueQuarterYoY":revenue_q_yoy,"earningsQuarterYoY":earnings_q_yoy,
        "revenueTTMGrowth":revenue_ttm_growth,"earningsTTMGrowth":earnings_ttm_growth,
        "revenueFYGrowth":revenue_fy_growth,"earningsFYGrowth":earnings_fy_growth,
        ...
        "dataSource":"Yahoo Finance financial statements (secondary) — currency/period normalized by dashboard"
    }
    result["sanityFlags"]=_sanity_flags(result)
    result["reconciliation"]=_reconcile_secondary(info,result)
    return result
...
def fundamental_interpretation(f):
    ...
    positives=[]; risks=[]
    rg=f.get("revenueTTMGrowth",f.get("revenueGrowth",np.nan))
    eg=f.get("earningsTTMGrowth",f.get("earningsGrowth",np.nan))
    ...
    if pd.notna(rg):
        if rg>0.10: positives.append("Pendapatan TTM tumbuh cukup kuat.")
        elif rg<0: risks.append("Pendapatan TTM sedang menurun.")
    if pd.notna(eg):
        if eg>0.10: positives.append("Laba TTM tumbuh cukup kuat.")
        elif eg<0: risks.append("Laba TTM sedang menurun.")
    if pd.notna(roe):
    ...
    if pd.notna(fcf) and fcf<0:
        risks.append("Free cash flow negatif.")
    if pd.notna(pe) and pe<=0:
        risks.append("PER tidak bermakna/negatif karena laba tidak positif.")
    if pd.notna(pb) and pb>5:
        risks.append("PBV tinggi; perlu dibandingkan dengan sektor dan historis.")

    for flag in f.get("sanityFlags",[]):
        risks.append("Sanity check: "+flag)

    if positives and len(positives)>=len(risks)+2:
        summary="Fundamental terlihat cukup sehat dari data statement sekunder yang sudah dinormalisasi."
...
def fundamental_completeness(f):
    if not f:
        return 0.0,"Data terbatas"
    fields=[
        "revenue","netIncome","equity","totalDebt","totalCash",
        "returnOnEquity","profitMargins","trailingPE","priceToBook",
        "operatingCashflow","freeCashflow"
    ]
    vals=[f.get(k,np.nan) for k in fields]
    c=sum(pd.notna(v) for v in vals)/len(vals)
    label="Lengkap" if c>=0.85 else "Sebagian" if c>=0.55 else "Terbatas"
    return c,label

def sector_profile(f):
    sector=str((f or {}).get("sector","N/A")).lower()
    industry=str((f or {}).get("industry","N/A")).lower()

    if "financial" in sector or "bank" in industry:
        return {
            "group":"Bank / Financial",
            "focus":"ROE, PBV, pertumbuhan laba, kualitas modal dan aset.",
            "available":["ROE","PBV","Pertumbuhan laba","Market cap"],
            "missing":["NIM","CASA","LDR","CAR","NPL","CoC"],
            "note":"Rasio khusus bank seperti NIM/CASA/NPL belum tersedia dari feed otomatis saat ini."
        }
    if any(k in sector for k in ["energy","basic materials"]) or any(k in industry for k in ["coal","oil","gas","mining","metals"]):
        return {
            "group":"Komoditas / Energi",
            "focus":"Pertumbuhan laba, margin, leverage, cash flow, EV/EBITDA, sensitivitas siklus.",
            "available":["Growth","Margin","DER","EV/EBITDA","Cash flow"],
            "missing":["Volume produksi","ASP","Cash cost","Cadangan"],
            "note":"Data operasional komoditas perlu sumber laporan emiten/produksi resmi."
        }
    if "technology" in sector or "internet" in industry or "software" in industry:
        return {
            "group":"Teknologi",
            "focus":"Pertumbuhan pendapatan, margin, cash burn/free cash flow, kas, valuasi penjualan.",
            "available":["Revenue growth","Margin","FCF","Cash","EV/Sales"],
            "missing":["GMV/GTV","Take rate","Adjusted EBITDA"],
            "note":"Metrik operasional teknologi perlu laporan emiten."
        }
    if any(k in sector for k in ["consumer","communication","industrials","healthcare"]):
        return {
            "group":"Operasional / Konsumer",
            "focus":"Pertumbuhan, margin, ROE, DER, cash flow, PER/PBV.",
            "available":["Growth","Margin","ROE","DER","Cash flow","PER/PBV"],
            "missing":[],
            "note":"Gunakan pembanding sektor/historis untuk menilai murah/mahal."
        }
    return {
        "group":"Umum",
        "focus":"Pertumbuhan, profitabilitas, leverage, cash flow, valuasi.",
        "available":["Growth","ROE","DER","Cash flow","PER/PBV"],
        "missing":[],
        "note":"Interpretasi tetap perlu disesuaikan model bisnis."
    }

def validation_status(f):
    if not f:
        return "Belum tersedia",[]
    flags=list(f.get("sanityFlags",[]))
    recon=f.get("reconciliation",{})
    big=[k for k,v in recon.items() if v.get("status")=="Berbeda besar"]
    if flags or len(big)>=2:
        return "Perlu verifikasi",flags+[f"Rekonsiliasi berbeda besar: {', '.join(big)}"] if big else flags
    if big:
        return "Cukup, ada selisih", [f"Rekonsiliasi berbeda besar: {', '.join(big)}"]
...
def golden_cross_snapshot(tf):
    if tf is None or tf.empty:
        return {
            "status":"Data belum cukup","days":np.nan,"date":None,
            "active":False,"alignment":"Belum tersedia","confirmation":"Belum tersedia"
        }

    valid=tf.dropna(subset=["SMA50","SMA200"]).copy()
    if valid.empty:
        return {
            "status":"Data belum cukup","days":np.nan,"date":None,
            "active":False,"alignment":"Belum tersedia","confirmation":"Belum tersedia"
    ...
    last=valid.iloc[-1]
    active=bool(last["SMA50"]>last["SMA200"])

    events=valid.loc[valid["GoldenCrossEvent"]]
    deaths=valid.loc[valid["DeathCrossEvent"]]

    latest_golden_date=events["Date"].iloc[-1] if not events.empty else None
    latest_death_date=deaths["Date"].iloc[-1] if not deaths.empty else None

    if active:
        if latest_golden_date is not None:
            days=int((pd.Timestamp(last["Date"])-pd.Timestamp(latest_golden_date)).days)
            status="Golden Cross baru" if days<=20 else "Golden Cross masih aktif"
            cross_date=latest_golden_date
        else:
            days=np.nan
            status="SMA50 di atas SMA200"
            cross_date=None
    else:
        if latest_death_date is not None:
            days=int((pd.Timestamp(last["Date"])-pd.Timestamp(latest_death_date)).days)
            status="Death Cross baru" if days<=20 else "Death Cross masih aktif"
            cross_date=latest_death_date
        else:
            days=np.nan
            status="Belum Golden Cross"
            cross_date=None

    if bool(last.get("BullMAAlignment",False)):
        alignment="Bullish kuat: Harga > SMA20 > SMA50 > SMA200"
    elif bool(last.get("BearMAAlignment",False)):
        alignment="Bearish kuat: Harga < SMA20 < SMA50 < SMA200"
    elif active:
        alignment="Bullish jangka panjang, tetapi susunan MA belum sempurna"
    else:
        alignment="Belum bullish jangka panjang"

    adx=last.get("ADX14",np.nan)
    rvol=last.get("RVOL20",np.nan)
    if active and pd.notna(adx) and adx>=20 and pd.notna(rvol) and rvol>=1:
        confirmation="Terkonfirmasi cukup baik oleh tren dan volume"
    elif active and pd.notna(adx) and adx>=20:
        confirmation="Tren cukup kuat, tetapi volume belum kuat"
    elif active:
        confirmation="Cross aktif, tetapi konfirmasi tren/volume masih lemah"
    else:
        confirmation="Tidak ada konfirmasi Golden Cross aktif"
...
def golden_cross_plain_language(gc):
    if gc["status"]=="Golden Cross baru":
        return "SMA50 baru menembus ke atas SMA200. Ini sinyal perubahan tren jangka menengah-panjang, tetapi tetap perlu konfirmasi."
    if gc["status"]=="Golden Cross masih aktif":
        return "SMA50 masih berada di atas SMA200. Tren jangka panjang masih relatif positif, tetapi sinyal bukan lagi sinyal baru."
    if gc["status"]=="Death Cross baru":
        return "SMA50 baru turun ke bawah SMA200. Ini peringatan bahwa tren jangka menengah-panjang sedang melemah."
    if gc["status"]=="Death Cross masih aktif":
        return "SMA50 masih berada di bawah SMA200. Struktur tren jangka panjang masih lemah."
    if gc["status"]=="SMA50 di atas SMA200":
        return "SMA50 berada di atas SMA200, tetapi titik cross awal tidak terlihat dalam histori yang tersedia."
    return "Belum ada Golden Cross aktif pada data yang tersedia."


def _forward_return_series(g,horizon):
    x=g.sort_values("Date").copy()
    x[f"Fwd{horizon}"]=x["Close"].shift(-horizon)/x["Close"]-1
    return x

@st.cache_data
def build_backtest_events(price,own):
    rows=[]
    for ticker,g in price.groupby("Ticker"):
        ...
        # Forward returns
        for h in [20,60,120]:
            tf[f"Fwd{h}"]=tf["Close"].shift(-h)/tf["Close"]-1

        # Golden / Death cross events
        for _,r in tf[tf["GoldenCrossEvent"]==True].iterrows():
            rows.append({
                "Ticker":ticker,"Date":r["Date"],"Signal":"Golden Cross",
                "Fwd20":r["Fwd20"],"Fwd60":r["Fwd60"],"Fwd120":r["Fwd120"]
            })
        for _,r in tf[tf["DeathCrossEvent"]==True].iterrows():
            rows.append({
                "Ticker":ticker,"Date":r["Date"],"Signal":"Death Cross",
                "Fwd20":r["Fwd20"],"Fwd60":r["Fwd60"],"Fwd120":r["Fwd120"]
            })

        # Ownership snapshot events aligned to first available trading date >= snapshot
        ot=own[(own["Ticker"]==ticker)&own["DeltaRetail"].notna()].sort_values("Date")
        if not ot.empty:
            for _,o in ot.iterrows():
                candidates=tf[tf["Date"]>=o["Date"]]
                if candidates.empty:
                    continue
                r=candidates.iloc[0]
                if o["DeltaRetail"]<0:
                    sig="Retail Turun"
                elif o["DeltaRetail"]>0:
                    sig="Retail Naik"
                else:
                    sig="Retail Tetap"
                rows.append({
                    "Ticker":ticker,"Date":o["Date"],"Signal":sig,
                    "Fwd20":r["Fwd20"],"Fwd60":r["Fwd60"],"Fwd120":r["Fwd120"]
                })
    return pd.DataFrame(rows)

def backtest_summary(events,signal):
    e=events[events["Signal"]==signal].copy()
    out=[]
    for h in [20,60,120]:
        col=f"Fwd{h}"
        v=e[col].dropna()
        if len(v)==0:
            out.append({"Horizon":f"{h}D","N":0,"Avg":np.nan,"Median":np.nan,"HitRate":np.nan})
        else:
            out.append({
                "Horizon":f"{h}D","N":len(v),
                "Avg":v.mean(),"Median":v.median(),"HitRate":(v>0).mean()
            })
    return pd.DataFrame(out)

# ------------------------------------------------------------
# 8: Daily workflow helpers
# ------------------------------------------------------------
def daily_shortlist(scr,limit=20):
    if scr.empty:
        return scr
    x=scr.copy()
    x["DailyPriority"]=x["EvidenceScore"].fillna(0)
    x.loc[x["Trend"].isin(["Naik kuat","Cenderung naik"]),"DailyPriority"]+=1
    x.loc[x["VolRatio"].fillna(0)>=1.5,"DailyPriority"]+=1
    x.loc[x["RiskOverride"]=="Ya","DailyPriority"]-=2
    return x.sort_values(["DailyPriority","EvidenceScore","VolRatio"],ascending=[False,False,False]).head(limit)
...
tab1,tab2,tab3,tab4,tab5,tab6,tab7,tab8,tab9,tab10,tab11=st.tabs([
    "⭐ Pilihan Hari Ini","🔎 Semua Saham","📊 Detail Saham","🏢 Fundamental",
    "📰 Berita & Sentimen","🧾 Final Research Card","💼 Portfolio",
    "🧪 Fundamental Validation","📐 Backtest Lab","🗓️ Daily Workflow","🌐 Kondisi Pasar"
])
...
with tab3:
    ...
    if p.empty:
        ...
        st.markdown("### Analisis Moving Average & Golden Cross")
        gc=golden_cross_snapshot(tf)
        g1,g2,g3,g4=st.columns(4)
        g1.metric("Status cross",gc["status"])
        g2.metric("SMA50","N/A" if pd.isna(last["SMA50"]) else rupiah(last["SMA50"]))
        g3.metric("SMA200","N/A" if pd.isna(last["SMA200"]) else rupiah(last["SMA200"]))
        g4.metric("Umur sinyal","N/A" if pd.isna(gc["days"]) else f"{gc['days']} hari")
        st.info(golden_cross_plain_language(gc))
        st.write(f"**Susunan MA:** {gc['alignment']}")
        st.write(f"**Konfirmasi:** {gc['confirmation']}")
        with st.expander("📘 Apa itu Golden Cross?"):
            st.write("**Golden Cross** terjadi ketika SMA50 menembus naik SMA200. Ini biasanya dibaca sebagai perbaikan tren jangka menengah-panjang.")
            st.write("**Death Cross** adalah kebalikannya: SMA50 menembus turun SMA200.")
            st.write("Golden Cross bukan jaminan harga akan naik. Sinyal lebih berkualitas jika harga juga berada di atas SMA20/50/200, ADX cukup kuat, dan volume mendukung.")
            st.write("Dashboard menggunakan data EOD/final untuk menghindari sinyal cross palsu dari candle intraday yang belum selesai.")
...
with tab4:
    ...
    if not f:
        ...
        st.caption(
            ...
            f"Mata uang laporan terdeteksi: **{f.get('financialCurrency','N/A')}** · "
            f"Mata uang harga: **{f.get('priceCurrency','N/A')}** · "
            f"Sumber otomatis: **{f['dataSource']}**"
        )
        if f.get("reportedFinancialCurrency")!=f.get("financialCurrency"):
            st.warning(
                f"🧭 Metadata Yahoo melaporkan **{f.get('reportedFinancialCurrency')}**, "
                f"tetapi engine menginfer **{f.get('financialCurrency')}** karena hasil rasio jauh lebih konsisten. "
                f"Metode: {f.get('currencyDetectionMethod')}."
            )
        else:
            st.caption(f"Deteksi mata uang: **{f.get('currencyDetectionMethod','metadata')}**.")

        if f.get("financialCurrency")!=f.get("priceCurrency"):
            if f.get("fxApplied"):
                st.success(
                    f"💱 Currency normalization aktif: {f.get('financialCurrency')} → {f.get('priceCurrency')} "
                    f"(FX flow ~{f.get('fxFlow',np.nan):,.2f}, FX balance ~{f.get('fxBalance',np.nan):,.2f})."
                )
            else:
                st.error("⚠️ Mata uang laporan berbeda dengan harga saham dan FX belum berhasil diterapkan.")
        ...
        g1,g2,g3,g4=st.columns(4)
        g1.metric("Revenue Quarter YoY",fmt_percent(f.get("revenueQuarterYoY",np.nan)))
        g1.caption("Kuartal terbaru dibanding kuartal yang sama tahun lalu.")
        g2.metric("Net Profit Quarter YoY",fmt_percent(f.get("earningsQuarterYoY",np.nan)))
        g2.caption("Laba kuartal terbaru dibanding kuartal yang sama tahun lalu.")
        g3.metric("Revenue TTM Growth",fmt_percent(f.get("revenueTTMGrowth",np.nan)))
        g3.caption("Total 4 kuartal terakhir dibanding 4 kuartal sebelumnya.")
        g4.metric("Net Profit TTM Growth",fmt_percent(f.get("earningsTTMGrowth",np.nan)))
        g4.caption("Total laba 4 kuartal terakhir dibanding 4 kuartal sebelumnya.")

        gg1,gg2,gg3,gg4=st.columns(4)
        gg1.metric("Revenue FY Growth",fmt_percent(f.get("revenueFYGrowth",np.nan)))
        gg1.caption("Tahun buku terakhir dibanding tahun buku sebelumnya.")
        gg2.metric("Net Profit FY Growth",fmt_percent(f.get("earningsFYGrowth",np.nan)))
        gg2.caption("Laba tahun buku terakhir dibanding tahun sebelumnya.")
        gg3.metric("ROE",fmt_percent(f["returnOnEquity"]))
        gg3.caption("Kemampuan perusahaan menghasilkan laba dari modal pemegang saham.")
        gg4.metric("Margin laba bersih",fmt_percent(f["profitMargins"]))
        gg4.caption("Berapa bagian penjualan yang akhirnya menjadi laba bersih.")
        ...
        st.markdown("### Sanity Check & Rekonsiliasi")
        flags=f.get("sanityFlags",[])
        if flags:
            for flag in flags:
                st.error("⚠️ "+flag)
        else:
            st.success("✅ Tidak ada anomali unit/currency ekstrem yang terdeteksi pada rasio utama.")

        recon=f.get("reconciliation",{})
        if recon:
            st.caption("Pembanding di bawah hanya dari Yahoo info sebagai **secondary sanity check**, bukan sumber resmi.")
            rows=[]
            for metric,d in recon.items():
                rows.append({
                    "Metrik":metric,
                    "Dashboard":d["calculated"],
                    "Pembanding Sekunder":d["secondary"],
                    "Selisih Relatif":d["diff"],
                    "Status":d["status"]
                })
            rdf=pd.DataFrame(rows)
            rdf["Dashboard"]=rdf["Dashboard"].map(lambda x:f"{x:.2f}" if pd.notna(x) else "")
            rdf["Pembanding Sekunder"]=rdf["Pembanding Sekunder"].map(lambda x:f"{x:.2f}" if pd.notna(x) else "")
            rdf["Selisih Relatif"]=rdf["Selisih Relatif"].map(lambda x:f"{x:.0%}" if pd.notna(x) else "")
            st.dataframe(rdf,use_container_width=True,hide_index=True)
        else:
            st.info("Belum ada rasio pembanding sekunder yang cukup untuk rekonsiliasi otomatis.")

        profile=sector_profile(f)
        comp,complabel=fundamental_completeness(f)
        st.markdown("### Profil analisis sektoral")
        st.info(f"**{profile['group']}** — {profile['focus']} {profile['note']}")
        if profile["missing"]:
            st.caption("Belum tersedia otomatis: **"+", ".join(profile["missing"])+"**")

        st.markdown("### Kualitas data")
        st.metric("Kelengkapan fundamental",f"{comp:.0%}",complabel)
        completeness_fields=[
        ...
        with st.expander("📘 Penjelasan istilah fundamental"):
            ...
            st.write("**Quarter YoY** — kuartal terbaru dibanding kuartal yang sama tahun lalu.")
            st.write("**TTM Growth** — total 4 kuartal terakhir dibanding total 4 kuartal sebelumnya.")
            st.write("**FY Growth** — tahun buku terakhir dibanding tahun buku sebelumnya.")
            st.write("**Currency normalization** — mengubah angka laporan dan market cap ke mata uang yang sama sebelum menghitung valuasi.")
            st.write("**Sanity check** — peringatan otomatis jika rasio terlalu ekstrem dan berpotensi disebabkan salah unit, mata uang, atau periode.")
            st.write("**Currency inference** — jika metadata mata uang menghasilkan rasio tidak masuk akal, engine mencoba kandidat lain (terutama USD untuk emiten IDX) dan memilih yang paling konsisten dengan pembanding sekunder.")
...
with tab6:
    ...
    if rp.empty:
        ...
        rgc=golden_cross_snapshot(rtf)
        if rgc["status"]=="Golden Cross baru":
            tscore+=1
            tpos.append("Golden Cross baru terdeteksi.")
        elif rgc["status"]=="Golden Cross masih aktif":
            tpos.append("Golden Cross masih aktif.")
        elif rgc["status"] in ("Death Cross baru","Death Cross masih aktif"):
            tscore-=1
            trisk.append(rgc["status"]+".")
        ...
        with c1:
            with st.container(border=True):
                ...
                if rf:
                    st.caption(
                        f"{rf.get('financialCurrency','N/A')} → {rf.get('priceCurrency','N/A')} · "
                        f"{rf.get('statementBasis','N/A')}"
                    )
                if fpos:
        ...
        with c2:
            with st.container(border=True):
                ...
                st.write(f"**MA Cross:** {rgc['status']}")
...
with tab8:
    ...
    st.write(
        "Tab ini memeriksa apakah angka fundamental **masuk akal, konsisten antar-periode, "
        "dan tidak menunjukkan masalah currency/unit**."
    )
    vticker=st.selectbox(
        "Pilih saham untuk validasi",
        full_universe,
        index=full_universe.index("DSSA") if "DSSA" in full_universe else 0,
        key="validation_ticker"
    )
    with st.spinner(f"Memvalidasi {vticker}..."):
        vf=fetch_fundamental_snapshot(vticker)

    if not vf:
        st.warning("Data fundamental belum tersedia.")
    else:
        comp,complabel=fundamental_completeness(vf)
        vstat,vissues=validation_status(vf)
        profile=sector_profile(vf)

        v1,v2,v3,v4=st.columns(4)
        v1.metric("Kelengkapan",f"{comp:.0%}",complabel)
        v2.metric("Status validasi",vstat)
        v3.metric("Mata uang laporan",vf.get("financialCurrency","N/A"))
        v4.metric("Basis",vf.get("statementBasis","N/A"))

        st.markdown("### Profil sektoral")
        st.info(
            f"**{profile['group']}** — Fokus analisis: {profile['focus']} "
            f"{profile['note']}"
        )
        if profile["missing"]:
            st.caption("Metrik khusus sektor yang belum tersedia otomatis: **"+", ".join(profile["missing"])+"**")
        ...
        if vissues:
            ...
            st.success("Tidak ditemukan anomali besar pada currency/unit/reconciliation.")

        st.markdown("### Rekonsiliasi rasio sekunder")
        recon=vf.get("reconciliation",{})
        if recon:
            rr=[]
            ...
            rdf=pd.DataFrame(rr)
            rdf["Dashboard"]=rdf["Dashboard"].map(lambda x:f"{x:.2f}")
            rdf["Pembanding sekunder"]=rdf["Pembanding sekunder"].map(lambda x:f"{x:.2f}")
            rdf["Selisih"]=rdf["Selisih"].map(lambda x:f"{x:.0%}")
            st.dataframe(rdf,use_container_width=True,hide_index=True)
        else:
            st.info("Pembanding rasio sekunder belum tersedia.")

        st.caption(
            "Validasi ini bukan pengganti laporan resmi IDX/emiten. Tujuannya mendeteksi error otomatis sebelum angka masuk ke Research Card."
        )

with tab9:
    st.subheader("Backtest Lab")
    st.write(
        "Uji sederhana apakah sinyal historis di dashboard pernah diikuti return positif. "
        "Ini **bukan jaminan masa depan** dan belum memasukkan biaya transaksi/slippage."
    )
    events=build_backtest_events(price,own)
    if events.empty:
        st.warning("Belum cukup data untuk backtest.")
    else:
        signal=st.selectbox(
            "Pilih sinyal",
            ["Golden Cross","Death Cross","Retail Turun","Retail Naik"],
            key="backtest_signal"
        )
        bs=backtest_summary(events,signal)
        st.dataframe(
            bs.style.format({"Avg":"{:+.1%}","Median":"{:+.1%}","HitRate":"{:.0%}"}),
            use_container_width=True,hide_index=True
        )
        st.caption(
            "Hit rate = persentase event dengan return ke depan >0. "
            "Sampel kecil atau overlapping events harus dibaca hati-hati."
        )

        ev=events[events["Signal"]==signal].dropna(subset=["Fwd20"]).copy()
        if not ev.empty:
            fig=go.Figure(go.Box(y=ev["Fwd20"]*100,name=f"{signal} → 20D"))
            fig.update_layout(height=350,yaxis_title="Forward return 20D (%)")
            st.plotly_chart(fig,use_container_width=True)

with tab10:
    st.subheader("Daily Research Workflow")
    st.write(
        "Urutan pemakaian harian: **cek kondisi pasar → lihat shortlist → buka Research Card → "
        "cek risiko → baru susun rencana entry**."
    )

    dshort=daily_shortlist(scr,20)
    d1,d2,d3,d4=st.columns(4)
    d1.metric("Market regime",mreg)
    d2.metric("Shortlist",len(dshort))
    d3.metric("Kandidat Akumulasi",int((dshort["Status"]=="Kandidat Akumulasi").sum()) if not dshort.empty else 0)
    d4.metric("Risk override",int((dshort["RiskOverride"]=="Ya").sum()) if not dshort.empty else 0)

    if not dshort.empty:
        ddisp=dshort[[
            "Ticker","Status","Close","Ret20D","OwnershipText","VolumeText","Trend","MomentumText"
        ]].copy()
        ddisp.columns=["Kode","Status","Harga","20 Hari","Ownership","Volume","Arah Harga","Momentum"]
        ddisp["Harga"]=ddisp["Harga"].map(rupiah)
        ddisp["20 Hari"]=ddisp["20 Hari"].map(lambda x:"" if pd.isna(x) else f"{x:+.1%}")
        st.dataframe(ddisp,use_container_width=True,hide_index=True,height=500)

        csv_bytes=dshort.to_csv(index=False).encode("utf-8")
        st.download_button(
            "Download shortlist CSV",
            data=csv_bytes,
            file_name="daily_shortlist.csv",
            mime="text/csv"
        )

    with st.expander("📘 Checklist harian"):
        st.write("1. Pastikan market regime tidak berubah menjadi Pasar lemah.")
        st.write("2. Prioritaskan saham dengan evidence kuat tetapi jangan abaikan risk override.")
        st.write("3. Buka Final Research Card untuk melihat fundamental, ownership, technical, news, dan entry plan.")
        st.write("4. Cek apakah Golden Cross/MA alignment benar-benar dikonfirmasi ADX dan volume.")
        st.write("5. Hindari mengejar harga jika sudah jauh di atas support/entry zone.")
        st.write("6. Untuk berita material, buka artikelnya dan cek keterbukaan IDX/emiten.")
