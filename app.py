...
def fetch_news_snapshot(ticker,count=8):
    """
    Hybrid secondary-source news engine:
    1) yfinance Ticker.get_news
    2) yfinance Search
    3) Google News RSS Indonesia fallback
    """
    items=[]
    ...
    try:
        obj=yf.Ticker(f"{ticker}.JK")
        raw=obj.get_news(count=count,tab="news")
        if raw:
            items.extend(raw)
    except Exception:
        pass

    # 2. Yahoo/yfinance Search fallback
    if len(items)<3:
        try:
            cname=_company_name_for_news(ticker)
            search=yf.Search(f'{ticker} "{cname}" Indonesia saham',max_results=3,news_count=count)
    ...
    for item in items:
        ...
        if not p:
            continue
        key=(p["title"].strip().lower(),p.get("url"))
        if key in seen:
            continue
        seen.add(key)
        sentiment,materiality,category,impact=classify_news_text(p["title"],p["summary"])
        p.update({
            "sentiment":sentiment,
            "materiality":materiality,
            "category":category,
            "impact":impact,
            "dateText":_format_news_date(p["published"]),
            "engineSource":"Yahoo Finance/yfinance"
        })
        parsed.append(p)
        if len(parsed)>=count:
            break

    # 3. Google News RSS Indonesia fallback if Yahoo is sparse
    if len(parsed)<3:
        try:
            cname=_company_name_for_news(ticker)
            rss_items=_google_news_rss(ticker,cname,count=count)
            for p in rss_items:
                key=(p["title"].strip().lower(),p.get("url"))
                if key in seen:
                    continue
...
def news_overall_summary(news):
    if not news:
        return "Belum ada berita yang berhasil diambil.", "Netral"
    ...
    high_material=[]
    for n in news:
        w=weights.get(n["materiality"],1)
        total+=scores.get(n["sentiment"],0)*w
        denom+=w
        if n["materiality"]=="Tinggi":
    ...
    if high_material:
        summary=f"Sentimen berita {tone.lower()}. Ada {len(high_material)} berita berpotensi material yang perlu dibaca lebih dalam."
    else:
        summary=f"Sentimen berita {tone.lower()}. Belum terlihat berita material tinggi dari feed yang tersedia."
    return summary,tone
...
tab1,tab2,tab3,tab4,tab5,tab6=st.tabs(["⭐ Pilihan Hari Ini","🔎 Semua Saham","📊 Detail Saham","🏢 Fundamental","📰 Berita & Sentimen","🌐 Kondisi Pasar"])
...
with tab5:
    ...
    st.write(
        ...
        "Klasifikasi dilakukan otomatis dari judul/ringkasan berita sehingga bersifat **indikatif**, bukan pengganti membaca artikel."
    )
    ...
    if not news:
        st.warning(
            "Belum ada berita yang berhasil diambil dari Yahoo maupun Google News RSS. "
            "Ini tidak berarti emiten tidak memiliki berita; gunakan keterbukaan IDX/emiten sebagai pengecekan manual."
        )
    else:
        ...
        k1.metric("Berita ditemukan",len(news))
        k2.metric("Positif",sum(n["sentiment"]=="Positif" for n in news))
        k3.metric("Negatif",sum(n["sentiment"]=="Negatif" for n in news))
        k4.metric("Materialitas tinggi",sum(n["materiality"]=="Tinggi" for n in news))
        ...
        for n in news:
            ...
            with st.container(border=True):
                ...
                with c1:
                    ...
                    st.caption(f"{n['publisher']} · {n['dateText']} · via {n.get('engineSource','feed sekunder')}")
                    if n["summary"]:
