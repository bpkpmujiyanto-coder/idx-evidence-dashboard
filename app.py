...
def get_price_history(price,ticker,min_rows=120):
    """
    Freshness-aware price loader.

    Priority:
    1) Use local history when it is current.
    2) If local history is stale, fetch on-demand and use remote only when
       it contains a newer completed EOD date.
    3) Never replace a longer/current local dataset with an older remote one.
    """
    ...
    local_last=None
    if not local.empty:
        local_last=pd.to_datetime(local["Date"]).max()

    # A long history can still be stale. Check remote whenever the latest
    # local EOD is older than today's date. On weekends/holidays the remote
    # dataset will simply return the same latest trading date.
    now=pd.Timestamp.now(tz=ZoneInfo("Asia/Jakarta"))
    needs_freshness_check=(
        local.empty or
        local_last is None or
        pd.Timestamp(local_last).date() < now.date()
    )

    remote=pd.DataFrame()
    if needs_freshness_check or len(local)<min_rows:
        remote=fetch_ticker_on_demand(ticker)

    if not remote.empty:
        remote_last=pd.to_datetime(remote["Date"]).max()
        if local.empty:
            return remote,"Histori terbaru diambil saat dibuka"
