...
def fetch_news_snapshot(ticker,count=8):
    """
    Uses yfinance get_news and Search as fallback. Secondary source only.
    """
    items=[]
    try:
        obj=yf.Ticker(f"{ticker}.JK")
    ...
    # Fallback search can surface company/ticker-related news even when Ticker.news is sparse.
    if len(items)<3:
        try:
            search=yf.Search(f"{ticker} Indonesia stock",max_results=3,news_count=count)
...
with tab5:
    ...
    st.caption(
        "Sumber otomatis: Yahoo Finance/yfinance (secondary news feed). "
        "Untuk aksi korporasi, laporan keuangan, transaksi material, suspensi, atau isu hukum, "
        "cek keterbukaan informasi IDX/emiten sebagai sumber utama."
    )

    if not news:
        st.warning("Feed berita belum memberikan hasil untuk ticker ini.")
    else:
        # KPIs
        ...
        for n in news:
            ...
            with st.container(border=True):
                ...
                with c1:
                    ...
                    st.caption(f"{n['publisher']} · {n['dateText']}")
                    if n["summary"]:
